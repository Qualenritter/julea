...
backend_dataset_create(const char* name, char* parent, bson_t* bson, char* key)
{
	...
	{ // insert new dataset
		sqlite3_prepare_v2(backend_db, "INSERT INTO smd (name,meta_type,parent_key,file_key,ndims,dims0,dims1,dims2,dims3,distribution) VALUES (?,?,?,?,?,?,?,?,?,?);",
			-1, &stmt, NULL);
		sqlite3_bind_text(stmt, 1, name, -1, NULL);
...
backend_dataset_open(const char* name, char* parent, bson_t* bson, char* key)
{
	...
	bson_init(bson);
	g_return_val_if_fail(name != NULL, FALSE);
	sqlite3_prepare_v2(backend_db, "SELECT key, ndims, dims0, dims1, dims2, dims3, distribution FROM smd WHERE name = ? AND parent_key = ?;", -1, &stmt, NULL);
	sqlite3_bind_text(stmt, 1, name, -1, NULL);
	sqlite3_bind_int64(stmt, 2, *((sqlite3_int64*)parent));
	ret = sqlite3_step(stmt);
	memset(key, 0, SMD_KEY_LENGTH);
