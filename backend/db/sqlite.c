...
j_sql_finalize(sqlite3* backend_db, void* _stmt, GError** error)
{
	...
	g_debug("%s %p %p", G_STRFUNC, backend_db, _stmt);
...
j_sql_prepare(sqlite3* backend_db, const char* sql, void* _stmt, GArray* types_in, GArray* types_out, GError** error)
{
	...
	g_debug("_%s %p %s", G_STRFUNC, backend_db, sql);
	...
	g_debug("%s %p %p %s", G_STRFUNC, backend_db, *stmt, sql);
	return TRUE;
_error:
...
{
	J_TRACE_FUNCTION(NULL);
	g_debug("%s %p", G_STRFUNC, backend_db);
	sqlite3_stmt* stmt;
	gboolean found;
...
j_sql_open(void)
{
	...
	g_debug("j_sql_open %p", backend_db);
	return backend_db;
_error:
...
j_sql_close(sqlite3* backend_db)
{
	...
	g_debug("j_sql_close %p", backend_db);
