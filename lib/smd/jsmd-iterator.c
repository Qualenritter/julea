...
{guint ret;
	...
	iterator = g_slice_new(JSMDIterator);
	iterator->schema = j_smd_schema_ref(schema, error);
	j_goto_error_subcommand(!iterator->schema);
	iterator->selector = j_smd_selector_ref(selector, error);
	j_goto_error_subcommand(!iterator->selector && selector);
	iterator->iterator = NULL;
	iterator->ref_count = 1;
	iterator->valid = FALSE;
	iterator->bson_valid = FALSE;
	ret = j_smd_internal_query(schema->namespace, schema->name, &selector->bson, &iterator->iterator, batch, error);
	j_goto_error_subcommand(!ret);
	iterator->valid = TRUE;
	return iterator;
_error:
	j_smd_iterator_unref(iterator);
	return NULL;
}
JSMDIterator*
j_smd_iterator_ref(JSMDIterator* iterator, GError** error)
{
...
void
j_smd_iterator_unref(JSMDIterator* iterator)
{
	if (iterator && g_atomic_int_dec_and_test(&iterator->ref_count))
	{
		...
		g_slice_free(JSMDIterator, iterator);
	}
}
gboolean
...
{guint ret;
	...
	if (iterator->bson_valid)
		bson_destroy(&iterator->bson);
	ret = j_smd_internal_iterate(iterator->iterator, &iterator->bson, error);
	j_goto_error_subcommand(!ret);
...
gboolean
j_smd_iterator_get_field(JSMDIterator* iterator, gchar const* name, JSMDType* type, gpointer* value, guint64* length, GError** error)
{
	uint32_t len;
	...
	ret = j_smd_schema_get_field(iterator->schema, name, type, error);
	j_goto_error_subcommand(!ret);
