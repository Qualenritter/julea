...
main(gint argc, gchar** argv)
{
	...
	GOptionEntry entries[] = {
		...
		{ "smd-servers", 0, 0, G_OPTION_ARG_STRING, &opt_servers_smd, "Key-value servers to use", "host1,host2" },
		...
		{ "smd-backend", 0, 0, G_OPTION_ARG_STRING, &opt_smd_backend, "Key-value backend to use", "posix|null|gio|…" },
		{ "smd-component", 0, 0, G_OPTION_ARG_STRING, &opt_smd_component, "Key-value component to use", "client|server" },
		{ "smd-path", 0, 0, G_OPTION_ARG_STRING, &opt_smd_path, "Key-value path to use", "/path/to/storage" },
	...
	if ((opt_user && opt_system) //
		|| (opt_read && (opt_servers_object != NULL //
					...
					|| opt_kv_path != NULL //
					|| opt_smd_backend != NULL //
					|| opt_smd_component != NULL //
					|| opt_smd_path != NULL)) //
